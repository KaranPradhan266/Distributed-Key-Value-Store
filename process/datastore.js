    ...
    this.getRemappedData = function(index, max, maxReplicas) {
        ...
        Object.keys(store).forEach(function(key) {
            ...
            indexes.forEach(function(_index) {
                ...
                if (!(_index in stabilizationMetadata)) {
                    stabilizationMetadata[_index] = [];
                }

                stabilizationMetadata[_index].push({key: key, value: $this.get(key)});
            });
            // }
        });

        return stabilizationMetadata;
    }
}
